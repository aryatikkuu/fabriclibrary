 ...
 *
 *   npm run retag -- [--sample 20] [--budget 1]      # tag into reports/retag-looks.json (resumable)
 *   npm run retag -- --save [--groups detail] [--apply]
 *
 ...
 *
 * By default only fabrics the change can affect are re-tagged: checks,
 * stripes, geometrics, dots, textured solids, and swatch cards. --all does every fabric.
...
const REPORT = 'reports/retag-looks.json';
...
async function main() {
  ...
  const [fabrics, images, tags, verify] = await Promise.all([
    selectAll(db, 'fabrics', 'id, fabric_code'),
  ...
  const affected = new Set(tags.filter((t) => AFFECTED.includes(t.tag)).map((t) => t.fabric_id));
  ...
  console.log(`\n${rows.length} re-tagged, $${report.spend_usd.toFixed(3)} (${(report.spend_usd / rows.length || 0).toFixed(5)}/fabric); ` +
    `${rows.filter((r) => r.tags.detail.length).length} with a detail tag, ${rows.filter((r) => r.multi_fabric).length} swatch cards`);
}

/** Replace the re-tagged fabrics' tags in the chosen groups; all other tags are kept. */
async function save(report, apply) {
  const groups = String(flag('groups', 'detail')).split(',');
  if (groups.some((g) => !(g in visualTags))) throw new Error(`Unknown group in --groups ${groups}`);
  const rows = Object.values(report.fabrics);
  const tagRows = rows.flatMap((r) =>
    groups.flatMap((group) => (r.tags[group] ?? []).map((v) => ({ fabric_id: r.fabric_id, tag: `${group}:${v}` }))));
